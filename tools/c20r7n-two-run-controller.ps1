...
$r1=Join-Path $PackageDir 'c20r7n-run1.json'; $r2=Join-Path $PackageDir 'c20r7n-run2.json'; $summary=Join-Path $PackageDir 'c20r7n-summary.json'; $confirm=Join-Path $PackageDir 'CONFIRMATION_REQUIRED.txt'
...
if($SelfTest){
  ...
  if(!(Test-Path $r1)){
    ...
    if($LASTEXITCODE-ne0){throw 'RUN1_FAILED'}
...
"@ | Set-Content $confirm -Encoding utf8
  ...
  if(!(Test-Path $r2)){
    ...
    if($LASTEXITCODE-ne0){throw 'RUN2_FAILED'}
  }
}
